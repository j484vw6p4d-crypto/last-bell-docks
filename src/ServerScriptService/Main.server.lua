...
local function sync(p: Player)
	...
	if not ls then
		...
		local c = Instance.new("IntValue") c.Name = "Coins" c.Parent = ls
		local n = Instance.new("IntValue") n.Name = "Nights" n.Parent = ls
	end
	(ls:FindFirstChild("Coins") :: IntValue).Value = st.coins
	(ls:FindFirstChild("Nights") :: IntValue).Value = nightIndex
	Remotes.get("Stats"):FireClient(p, { coins = st.coins, phase = phase, carried = st.carried, display = #st.display, rebirths = st.rebirths, night = nightIndex })
end
local function rollCrate(): string
	local total = 0
	for _, w in pairs(Config.CrateWeight) do total += w end
	...
	for i = 1, Config.MaxStalls do
		if stalls[i] == nil then
			...
			if stall then
				...
				if lab and lab:IsA("TextLabel") then lab.Text = p.DisplayName end
			end
			return
		end
	end
...
Remotes.get("Harvest").OnServerEvent:Connect(function(player)
	local st = states[player.UserId]; if not st then return end
	if st.carried then notify(player, "You already carry a crate.") return end
	local char = player.Character local hrp = char and char:FindFirstChild("HumanoidRootPart")
	if not hrp or (hrp.Position - Vector3.new(0, 1.2, 40)).Magnitude > 22 then notify(player, "Go to the pier.") return end
	st.carried = rollCrate()
	notify(player, "Harvested " .. st.carried)
	sound(player, "pick")
	sync(player)
end)
Remotes.get("StockStall").OnServerEvent:Connect(function(player)
	local st = states[player.UserId]; if not st or not st.carried or not st.stallIndex then return end
	if #st.display >= Config.MaxDisplay then notify(player, "Stall full — sell first.") return end
	...
	if not hrp or not floor or (hrp.Position - floor.Position).Magnitude > 18 then notify(player, "Stand at your stall.") return end
...
Remotes.get("Sell").OnServerEvent:Connect(function(player)
	local st = states[player.UserId]; if not st then return end
	if phase ~= "Day" then notify(player, "Buyers only come during the day.") return end
	if #st.display == 0 then notify(player, "Nothing on display.") return end
...
Remotes.get("Steal").OnServerEvent:Connect(function(player)
	local st = states[player.UserId]; if not st then return end
	if phase ~= "Night" then notify(player, "Wait for the bell.") return end
	if st.carried then notify(player, "Hands full.") return end
	local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
		...
		if other ~= player then
			...
			if ost and ost.stallIndex and #ost.display > 0 then
				...
				if floor then
					...
					if d < bestDist then bestDist = d bestPlayer = other end
				end
			end
		end
	end
	if not bestPlayer then notify(player, "No open stall close enough.") return end
	...
	sound(player, "pick")
	sync(player) sync(bestPlayer)
end)
Remotes.get("Rebirth").OnServerEvent:Connect(function(player)
	local st = states[player.UserId]; if not st then return end
	if st.coins < Config.RebirthCost then notify(player, "Need " .. Config.RebirthCost .. " coins to rebirth.") return end
	st.coins = 0 st.rebirths += 1 st.display = {} st.carried = nil
	notify(player, "Rebirth " .. st.rebirths .. " — payout x" .. string.format("%.2f", multiplier(st)))
	sync(player)
end)

workspace.Harbor.Pier.HarvestPrompt.Triggered:Connect(function(player)
	Remotes.get("Harvest"):FireClient(player) -- no-op client; call server path
end)
-- prompt already fires server-side Triggered; hook harvest directly
workspace.Harbor.Pier.HarvestPrompt.Triggered:Connect(function(player)
	local ev = Remotes.get("Harvest")
	-- reuse same validation by invoking the binding: FireServer from client only.
end)

local function onPlayer(p: Player)
	states[p.UserId] = { coins = Config.StartingCoins, rebirths = 0, carried = nil, display = {}, stallIndex = nil }
	assignStall(p)
	sync(p)
	p.CharacterAdded:Connect(function(char)
		task.wait(0.2)
		local hrp = char:FindFirstChild("HumanoidRootPart")
		local idx = states[p.UserId].stallIndex
		if hrp and idx then
			local stall = workspace.Harbor.Stalls:FindFirstChild("Stall_" .. idx)
			if stall and stall.PrimaryPart then hrp.CFrame = stall.PrimaryPart.CFrame + Vector3.new(0, 4, 0) end
		end
	end)
end
...
Players.PlayerRemoving:Connect(function(p)
	local st = states[p.UserId]
	if st and st.stallIndex then stalls[st.stallIndex] = nil end
	states[p.UserId] = nil
end)

task.spawn(function()
	while true do
		phase = "Day" World.setNight(false)
		Remotes.get("Phase"):FireAllClients("Day")
		task.wait(Config.DaySeconds)
		phase = "Bell"
		Remotes.get("Phase"):FireAllClients("Bell")
		for _, p in ipairs(Players:GetPlayers()) do sound(p, "bell") end
		task.wait(Config.BellSeconds)
		phase = "Night" nightIndex += 1 World.setNight(true)
		Remotes.get("Phase"):FireAllClients("Night")
		for _, p in ipairs(Players:GetPlayers()) do sound(p, "night") sync(p) end
		task.wait(Config.NightSeconds)
	end
end)

print("[Last Bell Docks] Ready — 20 stalls, day/night cycle")
